                  ...
                  <div key={item.id} className={`bg-[#11141d] p-3 md:p-4 lg:p-5 rounded-[1.5rem] md:rounded-[2rem] lg:rounded-[2.5rem] transition-all duration-300 group relative flex flex-col border ${isOwned ? 'border-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'border-white/5 hover:border-blue-500/40'}`}>
                    ...
                    <Link href={`/item/${encodeURIComponent(item.id)}`} prefetch={false} className="flex-1">
                      <div className="aspect-square bg-black/20 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center p-3 md:p-4 mb-3 md:mb-4 relative overflow-hidden">
                        ...
                        <img loading="lazy" src={item.image} className="max-w-full max-h-full w-auto h-auto object-contain relative z-10 transition-transform group-hover:scale-110 duration-500" alt={item.name} />
                        <div className="absolute inset-0 flex items-center justify-center gap-1.5 z-20 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                          {/* Compare Button */}
                          <button 
                            onClick={(e) => {
                              e.preventDefault();
                            ...
                            className={`p-2 md:p-2.5 min-h-[44px] min-w-[44px] rounded-lg md:rounded-xl border bg-black/80 backdrop-blur-md transition-all pointer-events-auto ${compareList.find(i => i.id === item.id) ? 'text-blue-500 border-blue-500' : 'text-white border-white/10 hover:text-blue-500'}`} 
                          ...
                          {user?.steamId && (
                            <>
                              <button 
                                ...
                                className="p-2 md:p-2.5 min-h-[44px] min-w-[44px] rounded-lg md:rounded-xl border bg-black/80 backdrop-blur-md transition-all text-white border-white/10 hover:text-purple-500 hover:border-purple-500 pointer-events-auto"
                                title="Price Tracker"
                              ...
                              <button
                                ...
                                className={`p-2 md:p-2.5 min-h-[44px] min-w-[44px] rounded-lg md:rounded-xl border bg-black/80 backdrop-blur-md transition-all pointer-events-auto ${
