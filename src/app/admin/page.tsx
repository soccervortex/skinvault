  ...
  useEffect(() => {
    ...
    const loadChatAutomod = async () => {
      ...
      try {
        ...
        setAutomodError(e?.message || 'Failed to load automod settings');
      } finally {
    ...
    const loadChatFeed = async () => {
      ...
      try {
        ...
        setChatFeedMessages(last);
      } catch (e: any) {
        setChatFeedError(e?.message || 'Failed to load chat');
      } finally {
    ...
    const loadAutomodEvents = async () => {
      ...
      try {
        ...
        setAutomodEventsError(e?.message || 'Failed to load automod events');
      } finally {
        setAutomodEventsLoading(false);
      }
    };
    loadAutomodEvents();
    ...
    const automodEventsInterval = setInterval(() => {
    ...
    return () => {
      ...
      clearInterval(automodEventsInterval);
    };
      ...
      <div className="flex h-screen bg-[#08090d] text-white overflow-hidden font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-10 custom-scrollbar">
          <div className="w-full max-w-5xl mx-auto bg-[#11141d] border border-white/10 p-6 md:p-8 lg:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl space-y-6 md:space-y-8">
            ...
            <div className="mt-2">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 text-[10px] md:text-[11px] mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                ...
                >
                  ...
                  <div className="mt-2 text-[12px] font-black uppercase tracking-wider">Newsletter</div>
                </button>
  ...
  return (
    <div className="flex h-screen bg-[#08090d] text-white overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-10 custom-scrollbar">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 text-[10px] md:text-[11px]">
          <div className="bg-black/40 border border-blue-500/30 rounded-xl md:rounded-2xl p-3">
            <p className="text-blue-400 uppercase font-black tracking-[0.3em] mb-1 text-[9px]">
              Total Users
            </p>
        ...
        <div className="mt-8 pt-8 border-t border-white/10">
          ...
          <p className="text-[10px] md:text-[11px] text-gray-400 mb-4">
            View total referrals, milestone claims, credits granted, and top affiliates.
          </p>
          <button
            onClick={() => router.push('/admin/affiliate-stats')}
            className="w-full bg-blue-600 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20 flex items-center justify-center gap-2"
          >
            <Star size={14} /> Affiliate Analytics
          </button>
        </div>
            ...
            <div className="bg-black/40 border border-white/10 rounded-xl md:rounded-2xl p-4">
              <div className="flex items-center justify-between gap-3 mb-3">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    automodSettings?.enabled ? 'bg-blue-600' : 'bg-gray-600'
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      automodSettings?.enabled ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
              </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${
                        automodSettings?.blockLinks ? 'bg-blue-600' : 'bg-gray-600'
                      }`}
                    >
                      <span
                        className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${
                          automodSettings?.blockLinks ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  ...
                  <div key={String(m?.id || '')} className="bg-[#11141d] border border-white/10 rounded-xl p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <div className="text-[11px] font-black truncate">{m?.steamName || 'Unknown User'} <span className="text-[10px] text-gray-500 font-normal">({m?.steamId || ''})</span></div>
