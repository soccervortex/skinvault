...
export default function NewYearPromo({ steamId, onDismiss, onClaim }: NewYearPromoProps) {
  ...
  useEffect(() => {
    // Start animation: Fireworks throw gift (miss 2x, hit 3rd time)
  ...
  const handleClaim = async () => {
    setClaiming(true);
    try {
      router.push('/pro?promo=newyear2025&discount=200');
      onClaim();
    } catch (error) {
  ...
  if (!giftOpened) {
    return (
      <div className="fixed inset-0 z-[10000] pointer-events-none">
        {/* Gold/silver sparkle effect */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-yellow-400/10 to-silver-300/10 animate-pulse" 
             style={{
               background: 'linear-gradient(180deg, transparent 0%, rgba(255, 215, 0, 0.15) 50%, rgba(192, 192, 192, 0.15) 100%)',
               animation: 'aurora 4s ease-in-out infinite'
             }} />
        
        {/* Fireworks - coming from top right */}
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="relative bg-gradient-to-br from-yellow-500/90 to-yellow-600/90 border-4 border-yellow-300/30 rounded-3xl p-8 md:p-12 max-w-lg w-full shadow-2xl animate-scale-in">
        <button
          onClick={onDismiss}
        ...
        <div className="text-center space-y-6">
          ...
          <div className="bg-white/10 rounded-2xl p-6 border border-white/20">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <div className="text-center">
                ...
                <p className="text-3xl font-black text-yellow-200">€7.99</p>
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleClaim}
              disabled={claiming}
              className="flex-1 bg-white text-yellow-600 py-4 rounded-2xl font-black uppercase tracking-wider hover:bg-gray-100 transition-all shadow-xl flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {claiming ? (
                <>Bezig...</>
              ) : (
