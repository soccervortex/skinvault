...
export default function OldYearPromo({ steamId, onDismiss, onClaim }: OldYearPromoProps) {
  ...
  useEffect(() => {
    // Start animation: Subtle effect throws gift (miss 2x, hit 3rd time)
  ...
  const handleClaim = async () => {
    setClaiming(true);
    try {
      router.push('/pro?promo=oldyear2025&discount=200');
      onClaim();
    } catch (error) {
  ...
  if (!giftOpened) {
    return (
      <div className="fixed inset-0 z-[10000] pointer-events-none">
        {/* Subtle gray gradient effect */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-gray-500/5 to-gray-400/5 animate-pulse" 
             style={{
               background: 'linear-gradient(180deg, transparent 0%, rgba(169, 169, 169, 0.1) 50%, rgba(128, 128, 128, 0.05) 100%)',
               animation: 'aurora 4s ease-in-out infinite'
             }} />
        
        {/* Subtle particle - coming from top right */}
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="relative bg-gradient-to-br from-gray-700/90 to-gray-800/90 border-4 border-gray-400/30 rounded-3xl p-8 md:p-12 max-w-lg w-full shadow-2xl animate-scale-in">
        <button
          onClick={onDismiss}
        ...
        <div className="text-center space-y-6">
          ...
          <div className="bg-white/10 rounded-2xl p-6 border border-white/20">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <div className="text-center">
                ...
                <p className="text-3xl font-black text-gray-200">€7.99</p>
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleClaim}
              disabled={claiming}
              className="flex-1 bg-white text-gray-700 py-4 rounded-2xl font-black uppercase tracking-wider hover:bg-gray-100 transition-all shadow-xl flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {claiming ? (
                <>Bezig...</>
              ) : (
