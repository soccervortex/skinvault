...
export default function EasterPromo({ steamId, onDismiss, onClaim }: EasterPromoProps) {
  ...
  useEffect(() => {
    // Start animation: Bunny throws egg (miss 2x, hit 3rd time)
  ...
  const handleClaim = async () => {
    setClaiming(true);
    try {
      router.push('/pro?promo=easter2025&discount=200');
      onClaim();
    } catch (error) {
  ...
  if (!giftOpened) {
    return (
      <div className="fixed inset-0 z-[10000] pointer-events-none">
        {/* Pastel pink/blue gradient effect */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-pink-300/10 to-blue-300/10 animate-pulse" 
             style={{
               background: 'linear-gradient(180deg, transparent 0%, rgba(255, 179, 217, 0.15) 50%, rgba(179, 217, 255, 0.15) 100%)',
               animation: 'aurora 4s ease-in-out infinite'
             }} />
        
        {/* Bunny - coming from top right */}
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="relative bg-gradient-to-br from-pink-400/90 to-blue-400/90 border-4 border-pink-200/30 rounded-3xl p-8 md:p-12 max-w-lg w-full shadow-2xl animate-scale-in">
        <button
          onClick={onDismiss}
        ...
        <div className="text-center space-y-6">
          ...
          <div className="bg-white/10 rounded-2xl p-6 border border-white/20">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <div className="text-center">
                ...
                <p className="text-3xl font-black text-pink-200">€7.99</p>
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleClaim}
              disabled={claiming}
              className="flex-1 bg-white text-pink-500 py-4 rounded-2xl font-black uppercase tracking-wider hover:bg-gray-100 transition-all shadow-xl flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {claiming ? (
                <>Bezig...</>
              ) : (
