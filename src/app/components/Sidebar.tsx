    ...
    <>
          ...
          {user ? (
            <div className="bg-[#11141d] p-4 rounded-[2rem] border border-white/5 flex items-center gap-4 group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest truncate">{user.name}</p>
                <button 
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#11141d] border border-white/10 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl relative">
            <button onClick={() => setIsSearchOpen(false)} className="absolute top-8 right-8 text-gray-500 hover:text-white transition-colors"><X size={24}/></button>
            <h2 className="text-3xl font-black italic uppercase mb-2 tracking-tighter">Stalk Profile</h2>
            <p className="text-[10px] text-gray-500 uppercase font-black mb-8 tracking-[0.2em]">Enter a SteamID64 to see stats and vault</p>
            <form onSubmit={handleSearch} className="space-y-4">
              <input 
                ...
                className="w-full bg-black/40 border border-white/10 rounded-2xl py-5 px-8 text-sm font-black text-blue-500 outline-none focus:border-blue-500 transition-all placeholder:text-gray-800"
              />
              <button type="submit" className="w-full bg-blue-600 py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20">Analyze Combat Record</button>
            </form>
          </div>
        </div>
      )}
