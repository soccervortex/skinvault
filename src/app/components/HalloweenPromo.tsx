...
export default function HalloweenPromo({ steamId, onDismiss, onClaim }: HalloweenPromoProps) {
  ...
  useEffect(() => {
    // Start animation: Witch on broom throws gift (miss 2x, hit 3rd time)
  ...
  if (!giftOpened) {
    return (
      <div className="fixed inset-0 z-[10000] pointer-events-none">
        {/* Spooky orange/purple gradient effect */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-orange-500/10 to-purple-500/10 animate-pulse" 
             style={{
               background: 'linear-gradient(180deg, transparent 0%, rgba(255, 140, 0, 0.15) 50%, rgba(139, 0, 255, 0.15) 100%)',
               animation: 'aurora 4s ease-in-out infinite'
             }} />
        
        {/* Witch on broom - coming from top right */}
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="relative bg-gradient-to-br from-orange-600/90 to-purple-600/90 border-4 border-orange-300/30 rounded-3xl p-8 md:p-12 max-w-lg w-full shadow-2xl animate-scale-in">
        <button
          onClick={onDismiss}
        ...
        <div className="text-center space-y-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={handleClaim}
              disabled={claiming}
              className="flex-1 bg-white text-orange-600 py-4 rounded-2xl font-black uppercase tracking-wider hover:bg-gray-100 transition-all shadow-xl flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {claiming ? (
                <>Bezig...</>
              ) : (
