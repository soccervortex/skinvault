...
function resolveMilestone(milestoneId: string): Milestone | null {
  const id = String(milestoneId || '').trim();
  if (!id) return null;
  const base = BASE_MILESTONES.find((m) => m.id === id);
  if (base) return base;

  const m = id.match(/^ref_(\d+)$/);
  const n = m?.[1] ? Number(m[1]) : NaN;
  if (!Number.isFinite(n) || n < 7) return null;
  return { id: `ref_${n}`, referralsRequired: n, reward: { type: 'credits', amount: 100 } };
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const milestone = resolveMilestone(milestoneId);
